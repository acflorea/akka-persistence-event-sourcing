...
import org.kepteasy.common.Error
import org.kepteasy.domain.AggregateRoot
import org.kepteasy.domain.AggregateRoot.{Removed, Uninitialized}
...
object RequestHandler {
  ...
  /** Actor */
  case class CreateRequestActor[S <: AggregateRoot.State](
                                                           r: RequestContext,
                                                           target: ActorRef,
 ...
 * It then awaits response from [[AggregateManager]]
 * or handles exception or timeout if one of them occurred.
...
trait RequestHandler extends Actor with ActorLogging with Json4sSupport {
  ...
  /**
   * Processes result returned from aggregate
   * and completes the request using [[RequestHandler.complete]] method.
   */
...
/**
 * RequestHandlerCreator contains some helper methods
 * to create RequestHandlers for specific request types.
 */
trait RequestHandlerCreator {
  ...
  def handleRegister[S <: AggregateRoot.State](
                                                ...
                                                message: AggregateManager.Command)
                                              (implicit tag: ClassTag[S]) =
    actorRefFactory.actorOf(Props(CreateRequestActor[S](r, target, message)))
